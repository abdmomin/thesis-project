...
\usepackage[colorlinks,citecolor=Green,linkcolor=Blue,urlcolor=Blue]{hyperref}
...
% allow slightly looser line breaking instead of overfull lines
...
The outcome depends on the labeling budget, and the reasons are instructive. At the full budget, no strategy beats the baseline over three random seeds on WDC Products (best mean gain +0.016 F1, not significant), and on DBLP-Scholar only string augmentation changes the result at all: it makes it slightly worse (-0.008 F1). Even training on the union of all three additions, which overlap in zero pairs, only reproduces the best single strategy (+0.017 on WDC Products, statistically indistinguishable from string augmentation alone). But when the base training set is cut to half, the same LLM-labeled additions produce a significant gain on WDC Products (0.606 to 0.626 F1, p=0.0002). A string-augmentation control of matched size and class balance does not reproduce this gain (+0.000), so it comes from the teacher's labels, not from extra volume; at a quarter of the budget, the additions rescue a training set that otherwise collapses entirely. Automatic expansion is thus neutral where the human training set already covers the common patterns, and valuable where labels are scarce. Our analysis shows why: once the expansion is forced to be entity-disjoint from the test set, the candidate pool for LLM labeling contains almost no new positive pairs (5.6\% on WDC Products, 0\% on DBLP-Scholar), because on a closed benchmark nearly all true matches are already part of the labeled data. The teacher itself is not the problem: its labels agree with the ground truth in 97.1\% of cases (F1 0.862). Web retrieval does inject truly new positives, but the retrieved text comes from a different distribution than the benchmark records, and a follow-up experiment that rebalanced the web data and added LLM-generated hard negatives made results significantly worse rather than better. We place these findings in a detailed comparison with DistillER: Knowledge Distillation in Entity Resolution with Large Language Models  \cite{zeakis2026distiller} and with Labeling Training Data for Entity Matching Using Large Language Models \cite{steiner2026labeling}, whose label-replacement setting recovers over 99\% of the benchmark positives and therefore does not face the scarcity problem: expanding a training set is structurally harder than relabeling one. Along the way, the thesis documents an entity-leakage pitfall in augmentation pipelines, quantifies how single-seed evaluations produced conclusions that did not survive seed averaging, and argues for paired significance tests on shared test sets.
...
\item \textbf{RQ3:} Does web retrieval augmentation, which obtains truly new records from outside the benchmark and pairs them with training entities, improve downstream F1?
...
\item \textbf{A negative result with an explanation, and its boundary.} At the full labeling budget, no expansion strategy robustly improves the student. We trace this to three separate mechanisms: positive scarcity in entity-disjoint candidate pools (the benchmark already contains nearly all true matches among its entities), a text-distribution mismatch between web-retrieved records and benchmark records, and a performance ceiling on DBLP-Scholar that leaves no room for improvement. We consider the first mechanism the central finding of the thesis: \emph{expanding} a closed benchmark honestly is structurally harder than \emph{relabeling} it, because the discoverable new pairs are almost all negatives. A low-resource ablation then locates the boundary of the negative result: when the base training set is halved, the same LLM-labeled additions yield a significant gain (+0.019 F1, $p = 0.0002$) that a volume-matched string-augmentation control does not reproduce. The value of expansion is a function of label scarcity, not a fixed property of the data, and where it appears it is carried by the teacher's labels rather than by data volume.
...
This chapter first fixes terminology and describes the standard entity matching pipeline (Section~\ref{sec:preliminaries}). It then reviews the lines of work this thesis builds on: neural entity matching (Section~\ref{sec:rw_neural}), data augmentation (Section~\ref{sec:rw_augmentation}), large language models for entity matching (Section~\ref{sec:rw_llm}), knowledge distillation with LLM teachers (Section~\ref{sec:rw_distillation}), and active learning (Section~\ref{sec:rw_al}). To keep the review comparable across very different systems, we discuss each approach along four recurring dimensions: the matching model it uses, where its training labels come from, what it does to the training data (use it as given, perturb it, relabel it, or expand it), and the labeling budget at which it was evaluated. Section~\ref{sec:rw_positioning} collects the reviewed approaches in an overview table along these dimensions, together with their reported F1 scores, and states the gap this thesis addresses. Chapter~\ref{ch:comparison} comes back to these numbers at the end of the thesis and compares them with our own results in detail.
...
WDC Products \cite{peeters2024wdc} is a newer benchmark built from product offers on the web that carry schema.org annotations, which supply cluster identifiers as ground truth. What sets it apart is that difficulty is a controlled dimension: the benchmark comes in variants with a chosen share of corner cases (we use the hardest, ``80\% corner cases'' variant), with seen or unseen test entities (we use unseen, i.e.\ the entity-disjoint variant), and with three training sizes (we use small). This controllability is why WDC Products is our primary benchmark: it was explicitly designed to punish matchers that rely on surface similarity, which is the failure mode that data augmentation is supposed to address. It is also clearly harder than the older product benchmarks: a RoBERTa-based matcher trained on the large split of the 80\%-corner-case, unseen-test configuration reaches an F1 of about 0.72 \cite{peeters2024wdc,steiner2026labeling}, compared to Ditto's 0.87--0.89 on Walmart-Amazon and Abt-Buy.

\section{Data Augmentation for Entity Matching}
\label{sec:rw_augmentation}

Data augmentation enlarges a training set by transforming existing examples in label-preserving ways. In NLP, simple token-level operations such as synonym replacement, random insertion, swap, and deletion can help small-data text classification \cite{wei2019eda}; \citet{shorten2021text} survey the area. Ditto \cite{li2020deep} adapted this idea to entity matching with operators that respect the structure of serialized records: deleting spans of value tokens, shuffling token spans, dropping whole attributes, and swapping the two records of a pair. The intuition is that a matcher should not change its decision because a word was dropped or an attribute reordered, so training on such perturbed copies teaches invariance.

In Ditto's own ablations, augmentation is worth on the order of one F1 point on average, with larger gains on small training sets \cite{li2020deep}. Rotom \cite{miao2021rotom} took augmentation a step further with a meta-learning framework that learns which augmentation policies to apply and how to weight the generated examples, and reports that this beats fixed policies on entity matching among other tasks, again with the largest margins in low-resource settings. Our string-based strategy (Section~\ref{sec:method_string}) deliberately stays with Ditto's fixed operators: it is the simple, cheap, LLM-free baseline against which the two more elaborate strategies must justify their cost.

A shared limitation of all perturbation-based augmentation is that it cannot create truly new information. A perturbed copy of a pair contains the same entities, the same decisive attributes, and the same label; the model sees more variation but no new facts. The two other strategies in this thesis are attempts to break this limitation, by pairing entities in new combinations (LLM-based expansion) and by importing entirely new records from outside the benchmark (web retrieval).

\section{Large Language Models for Entity Matching}
\label{sec:rw_llm}

\citet{narayan2022can} first showed that large autoregressive language models can perform data integration tasks, including entity matching, from prompts with few or no examples. \citet{peeters2023using} evaluated ChatGPT on product matching and found it competitive with fine-tuned baselines that had seen thousands of labeled pairs, despite receiving no or only a handful of examples in the prompt; the extended study of \citet{peeters2025entity} systematically compared prompt designs, model families, and fine-tuning across benchmarks, and found that hosted frontier models match or exceed a fine-tuned RoBERTa on many datasets, with the practical drawbacks of cost and latency per pair.

These results changed the economics of the field. If an LLM can label a pair for a fraction of a cent at near-human quality, human annotation stops being the only way to create training data. Two consequences are being explored in the literature. The first is to use the LLM directly as the matcher, which is accurate but expensive at scale, since every candidate pair triggers an API call at inference time. The second is to use the LLM as an annotator that creates training data for a small student model, which is then cheap at inference time. This second route is the one this thesis takes, and it leads directly to knowledge distillation.
...
Table~\ref{tab:rw_overview} organizes the approaches reviewed in this chapter along four dimensions: the \emph{matching model} (feature-based classifier, neural matcher, pre-trained transformer, or prompted LLM), the \emph{label source} (human annotation or an LLM teacher), the \emph{data operation} (whether training pairs are used as given, perturbed, relabeled, or newly added), and the \emph{reported F1} on the benchmarks that overlap with this thesis. The F1 values are not directly comparable across rows, because training budgets and benchmark versions differ between the studies, but they show the shape of the field.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lllll}
    \toprule
    \textbf{Approach} & \textbf{Matcher} & \textbf{Label source} & \textbf{Data operation} & \textbf{Reported F1} \\
    \midrule
    Magellan & feature ML & human & as given & 0.923 DS, 0.491 AG \\
    DeepMatcher & RNN & human & as given & 0.947 DS, 0.693 AG \\
    Ditto & transformer & human & perturb & 0.956 DS, 0.756 AG \\
    Rotom & transformer & human & perturb (learned) & beats fixed policies \\
    Prompted LLMs & LLM & zero-/few-shot & none & $\approx$ fine-tuned student \\
    DistillER & RoBERTa student & LLM teacher & relabel ($\sim$10\%) & 0.86 DS, 0.57 WA \\
    Steiner \& Bizer & Ditto student & LLM teacher & relabel (full scale) & 0.938 DS, 0.707 WDC \\
    This thesis & RoBERTa student & human + LLM teacher & expand & 0.957 DS, 0.661 WDC \\
    \bottomrule
  \end{tabular}
  \caption{The reviewed approaches, categorized along the dimensions of this chapter (citations for each approach are given in the text), with reported F1 on benchmarks shared with this thesis. DS = DBLP-Scholar, AG = Amazon-Google, WA = Walmart-Amazon, WDC = WDC Products (80\% corner cases, unseen test set; small training split for this thesis). The numbers come from different training budgets and benchmark versions and are therefore only meaningful within each row's own study; Chapter~\ref{ch:comparison} makes the cross-study comparison explicit.}
  \label{tab:rw_overview}
\end{table}

Read along the first dimension, the table shows that the largest quality jumps came from changing the matching model: on DBLP-Scholar, F1 rises from 0.923 (Magellan) over 0.947 (DeepMatcher) to 0.956 (Ditto), and on Amazon-Google from 0.491 over 0.693 to 0.756. Everything after Ditto competes within a few F1 points on the same student architectures, which is why recent work has shifted its attention from the model to the training data. Read along the label-source and data-operation dimensions, the recent LLM-teacher works all sit in the \emph{relabel} rows: DistillER and \citet{steiner2026labeling} let the teacher label pairs the benchmark already contains, and both find that students trained on those machine labels stay close to students trained on human labels (0.69 versus 0.66 average F1 for DistillER's RoBERTa student; 0.707 versus 0.719 on WDC Products for \citealt{steiner2026labeling}). Perturbation-based augmentation \cite{li2020deep,miao2021rotom} occupies the \emph{perturb} rows and adds robustness but no new information. The \emph{expand} row, adding new labeled pairs to an existing human-labeled set, is empty before this thesis.

Against this backdrop, the gap this thesis addresses can be stated precisely. Prior work has shown that LLM teachers can \emph{replace} human annotators on existing benchmark pairs at small quality loss \cite{steiner2026labeling,zeakis2026distiller}, and that perturbation-based augmentation can help transformer matchers \cite{li2020deep,miao2021rotom}. What has not been studied is the \emph{expansion} setting: keeping the human training set and adding new, automatically discovered, LLM-labeled pairs on top, either from the benchmark's own entity tables via blocking, or from outside the benchmark via web retrieval. Expansion differs from replacement in what the candidate pool can contain, and it differs from perturbation in that it can, in principle, add new information. The thesis evaluates both new routes against the perturbation baseline under identical conditions, imposes the entity-disjointness hygiene that the setting requires, and explains the results through the composition of the candidate pools rather than only through downstream F1. At the end of the thesis, Chapter~\ref{ch:comparison} returns to the numbers in Table~\ref{tab:rw_overview} and works out, budget by budget, how our results relate to them.
...
DBLP-Scholar plays the role of a contrast dataset in this thesis. Fine-tuned transformers reach F1 around 0.95 on it, so there is little headroom; its pairs are mostly either clearly matching or clearly different (few corner cases, as quantified in Section~\ref{sec:results_layer2}); and its split has hygiene issues that the next section makes precise. Figure~\ref{fig:test_sim} makes the contrast between the two benchmarks visible before any model is trained: on the WDC Products test split, the similarity distributions of matches and non-matches lie almost on top of each other at low token overlap (surface similarity is nearly uninformative, by the benchmark's corner-case design), whereas on DBLP-Scholar the two classes separate cleanly, with only a thin overlap band. If augmentation helps anywhere, it should be on WDC Products; DBLP-Scholar tests whether augmentation at least does no harm near the ceiling.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/similarity_distribution_wdc-products_test.pdf}
  \hfill
  \includegraphics[width=0.49\textwidth]{figures/similarity_distribution_dblp-scholar_test.pdf}
  \caption{Token-Jaccard similarity of test pairs by label. Left (WDC Products): matches and non-matches overlap heavily at low similarity, so token overlap alone cannot separate the classes. Right (DBLP-Scholar): the classes are nearly separated, which anticipates both the 0.95+ F1 ceiling and the scarcity of corner cases.}
  \label{fig:test_sim}
\end{figure}
...
Serialized records must fit the student's input window, and Figure~\ref{fig:token_length} shows they mostly do: WDC Products records concentrate at 25--50 whitespace tokens, with a long tail from verbose product descriptions that accumulates at the length cap. Since a pair encodes both records, the student's limit of 256 subword tokens per pair (Section~\ref{sec:method_student}) truncates only the tail of that tail; the identifying attributes (brand, title, and the head of the description) come first in the serialization and survive truncation.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/wdc_token_length_distribution.pdf}
  \caption{Whitespace-token length of serialized WDC Products training records (left and right entity), split by pair label. Most records are short (25--50 tokens); the spike at the right edge collects the long-description tail. Length is essentially uninformative about the label.}
...
\textbf{WDC Products is clean.} No entity from the test or validation split appears in training, no pair is shared between splits by ID, and no pair is shared by text either. Test performance on WDC Products therefore measures generalization to fully unseen products.
...
An expanded training set should not just be bigger; it should be bigger in the right way. Based on the properties that make entity matching training sets effective, the pipelines were designed toward the following targets:
...
Figure~\ref{fig:pool_sim} shows the similarity profile of the resulting pools: by construction of the $k$-nearest-neighbor blocking, both are concentrated at high cosine similarity (mean 0.83 on WDC Products, 0.75 on DBLP-Scholar); these are exactly the look-alike pairs a matcher finds hard.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/candidate_pool_similarity_hist_wdc-products.png}
  \hfill
  \includegraphics[width=0.49\textwidth]{figures/candidate_pool_similarity_hist_dblp-scholar.png}
  \caption{Embedding cosine similarity of the filtered candidate pools (left: WDC Products, 4{,}722 pairs; right: DBLP-Scholar, 18{,}841 pairs). Nearest-neighbor blocking concentrates both pools at high similarity: plausible-looking pairs whose labels are not obvious from the surface.}
  \label{fig:pool_sim}
\end{figure}
...
The third strategy attacks the limitation that the first two cannot escape: everything so far recombines or perturbs records the benchmark already contains. Web retrieval brings in \emph{new} records from outside. The idea is simple: for a product in the training set, other shops publish offers for the same product with different wording; for a paper, other bibliographic sites publish variant records. Retrieving such documents and pairing them with the original entity yields pairs whose positive side really adds information. The pipeline has four steps.
...
This step is where the strategy's characteristic bias enters. Because the query is built from the entity's own brand and title, the search engine mostly returns pages about \emph{that product}, which is the point, but it means the harvested pairs are predominantly positives. The relevant slice for WDC Products came out at 81\% positive. This skew is inherent to retrieval by identity (searching for a product finds that product), and Chapter~\ref{ch:iteration} describes a follow-up experiment that tried to engineer the complementary hard negatives deliberately.
...
The fix has three parts. First, a single module now serves as the authoritative source of entity-to-split membership, handling a subtlety that makes an ad-hoc fix dangerous: WDC Products uses one shared ID space (the same entity may appear on either side of a pair, so an excluded entity must be filtered from both sides), while DBLP-Scholar uses two separate per-side ID spaces whose numeric IDs overlap (so filtering must be side-specific, or a DBLP ID would wrongly exclude an unrelated Scholar record). Second, both pipelines filter their pools through this module: a candidate pair survives only if both entities belong to the training split. Third, a verification script recounts, for every augmented training file, how many pairs touch a held-out entity, i.e.\ an entity appearing in validation or test but \emph{not} in training. The pass criterion is zero, and the final regenerated training sets meet it on both datasets. On WDC Products, whose benchmark splits are entity-disjoint, the augmented sets are therefore fully test-disjoint. On DBLP-Scholar, where most test entities also appear in training (Section~\ref{sec:data_hygiene}), the policy yields augmentation that is exactly as disjoint as the baseline itself: it introduces no entity the original training set did not already use, which is the strongest guarantee possible on a non-disjoint benchmark.
...
Table~\ref{tab:baselines} shows the baseline students trained on the unmodified training sets. On DBLP-Scholar, \texttt{roberta-base} reaches an F1 of 0.9559 $\pm$ 0.0019, the expected near-ceiling result for a transformer on this benchmark, with the memorization caveat from Section~\ref{sec:data_hygiene}; it also lands right on the 0.956 that Ditto reports for this dataset \cite{li2020deep} (Section~\ref{sec:rw_neural}). On WDC Products the class-weighted baseline reaches 0.659 $\pm$ 0.015; the run-to-run spread is an order of magnitude larger than on DBLP-Scholar (per-seed F1: 0.677, 0.649, 0.653), which quantifies how unstable small-data fine-tuning is on this benchmark and why single-seed comparisons on it are unreliable. Figure~\ref{fig:cm_wdc} shows the error structure of a representative WDC Products baseline: false positives outnumber false negatives by more than two to one (297 versus 124), the over-prediction pattern that the prior-shift analysis below explains.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\textwidth]{figures/confusion_matrix_wdc-products.pdf}
  \caption{Confusion matrix of the class-weighted WDC Products baseline (initial single-seed run; test split, $n = 4{,}500$). The model over-predicts the match class (297 false positives against 124 false negatives), consistent with a decision rule calibrated to the 20\% training prior and applied to an 11\% test prior.}
  \label{fig:cm_wdc}
\end{figure}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lcccrl}
    ...
    llm\_aug & 0.952 $\pm$ 0.006 & 0.961 $\pm$ 0.006 & 0.9566 $\pm$ 0.0006 & +0.001 & 0.728 (n.s.) \\
    web\_aug & 0.948 $\pm$ 0.006 & 0.966 $\pm$ 0.007 & 0.9567 $\pm$ 0.0005 & +0.001 & 1.000 (n.s.) \\
    \bottomrule
  \end{tabular}
...
\textbf{On DBLP-Scholar, the baseline is unbeaten, and one strategy reliably hurts.} String augmentation loses on all three seeds ($-$0.008 F1, $p = 0.0079$); the effect is tiny (Cohen's $h \approx 0.03$) and only nominal under Bonferroni correction, but its direction is consistent. The mechanism is visible in the precision--recall split: doubling the training set with perturbed copies pushes recall up (0.960 $\rightarrow$ 0.971) but costs more precision (0.952 $\rightarrow$ 0.927), a bad trade on a dataset where the baseline's errors are already concentrated in a thin band of truly ambiguous pairs (Section~\ref{sec:results_layer3}). Perturbation noise near the ceiling erases signal rather than adding robustness: some perturbed copies of hard negatives lose exactly the tokens that made them negatives (the risk anticipated in Section~\ref{sec:method_string}). The LLM and web strategies tie the baseline ($p = 0.73$ and $1.0$): 2{,}085 clean teacher-labeled hard negatives (LLM) and 742 web pairs changed nothing measurable.

Answering RQ1--RQ3 directly: at the full labeling budget, none of the three strategies improves the student at a level distinguishable from seed noise, on either dataset. The remainder of the thesis is about \emph{why} (the answer differs per strategy) and about where the picture changes: Section~\ref{sec:iter_lowresource} shows that the LLM strategy \emph{does} help significantly once the base training set is made scarce.
...
On \textbf{DBLP-Scholar}, students err on 1.6--2.0\% of test pairs, and the distribution is nearly invariant across all four training sets: 82--88\% are ambiguous variants, plus a consistent handful (7--12 pairs) of a distinctive high-similarity non-match type, namely the same paper title and authors at a conference and in a journal, distinguishable only by venue and year together. A representative example pairs ``Querying and Mining Data Streams: You Only Get One Look'' at SIGMOD 2002 against SIGMOD Record 2003: identical title, identical authors, different publication. This error type is exactly the hard-negative pattern that DBLP-Scholar's training data barely contains (Section~\ref{sec:results_layer2}), which motivated the targeted mining experiment of Section~\ref{sec:iter_hardneg}. The full per-run breakdown and further examples are given in Appendix~\ref{app:errors}.

\subsection{Error Overlap Between Students}
\label{sec:results_overlap}

Do the augmented students fail on the same pairs as the baseline, or on different ones? On WDC Products, the misclassified-pair sets of the baseline and the LLM-augmented student overlap at a Jaccard similarity of 0.47 in the single-seed runs; on the seed-averaged ensemble predictions, where seed-specific noise errors are washed out, the overlap rises to 0.62 (llm\_aug), 0.61 (string\_aug), and 0.66 (web\_aug). In words: roughly half to two thirds of each augmented student's errors are exactly the baseline's errors. No strategy carves out and fixes a distinct error class; each mostly re-fails the same hard pairs and trades a handful of errors at the margin. This is the pair-level counterpart of the Layer 1 ties directly: the models make largely the same mistakes, which is consistent with the augmented data adding no new decisive signal.

\subsection{Evaluation on the Hard Test Subset}
\label{sec:results_hardsubset}

The design requirements call for a downstream evaluation restricted to the corner-case pairs of the test set, the pairs where improvement would matter most. Applying the Layer 2 corner-case definition to the test splits yields 526 hard test pairs on WDC Products (455 hard positives, 71 hard negatives) and 177 on DBLP-Scholar (62 and 115). On the seed-averaged predictions, corner-subset F1 on WDC Products is 0.833 for the baseline versus 0.863 (string), 0.857 (llm), and 0.855 (web): all three augmentations sit slightly above the baseline on the hard subset, consistent with their recall gains, though the margins are of the same order as the effects that Layer 1 already judged non-significant. On DBLP-Scholar the corner-subset F1 is 0.770 (baseline), 0.776 (llm), 0.754 (web), and 0.731 (string); string augmentation is clearly worst exactly where precision matters, matching its Layer 1 regression. Two caveats keep this analysis descriptive: the subsets are small, and their class mix differs sharply from the full test split (86\% positive on WDC Products against 11\% overall), so these F1 values are comparable \emph{across students} but not with the full-test numbers.

\section{Summary of Findings}
\label{sec:results_summary}

\begin{enumerate}
\item At the full labeling budget, no expansion strategy improves the student at a level distinguishable from seed noise, on either benchmark (RQ1--RQ3). String augmentation has the best WDC Products mean (+0.016, n.s.) and is the only strategy with a significant effect anywhere: a tiny, consistent \emph{degradation} on DBLP-Scholar. (Section~\ref{sec:iter_lowresource} shows the full-budget qualifier matters: the LLM additions help significantly at half the budget.)
...
\item The added slices of the three strategies overlap in zero pairs; they are perfectly complementary, which leaves union training as a natural follow-up.
\item The augmented students make largely the same errors as the baseline: the error sets overlap at Jaccard 0.61--0.66 on the seed-averaged predictions (Section~\ref{sec:results_overlap}), and no strategy fixes a distinct error class. On the corner-case test subset, all three strategies sit marginally above the baseline on WDC Products and string augmentation is clearly worst on DBLP-Scholar (Section~\ref{sec:results_hardsubset}).
...
After reviewing the first-round results, a second experimental iteration built on what the analysis had revealed. Four work items resulted: mining the specific hard-negative type that DBLP-Scholar lacks (Section~\ref{sec:iter_hardneg}), fixing the skew of the web slice while injecting deliberately generated hard negatives on WDC Products (Section~\ref{sec:iter_webv2}), training on unions of the complementary augmentation sets (Section~\ref{sec:iter_union}), and testing whether augmentation helps more when the base training set is artificially shrunk (Section~\ref{sec:iter_lowresource}). Two returned clear negative results, and the union produced the best WDC Products mean without separating from string augmentation alone. But the low-resource ablation (Section~\ref{sec:iter_lowresource}) delivers the thesis's one unambiguous positive finding: the same LLM-labeled additions that tie the baseline at the full budget help significantly when the base training set is halved, and a volume-matched control pins the gain on the teacher's labels rather than on data quantity.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lccrl}
    ...
    \bottomrule
  \end{tabular}
  \caption{WDC Products: rebalanced web augmentation with generated hard negatives (3 seeds, class-weighted). $^{*}$Survives Bonferroni correction (one of only two comparisons in the thesis that do, alongside the 50\%-budget result of Section~\ref{sec:iter_lowresource}).}
  \label{tab:webv2}
\end{table}

The result (Table~\ref{tab:webv2}) is the opposite of the hypothesis: both variants are significantly \emph{worse} than the baseline, and the harder-negative-heavy variant is worse than the balanced one ($-$0.029, $p = 0.0024$, one of only two results in this thesis that survive Bonferroni correction). The error analysis on the seed-averaged predictions locates the damage precisely: false positives \emph{increased} from 216 (baseline) to 259 (balanced) to 283 (hard-neg-heavy), concentrated in the ambiguous-variant class. Adding verified hard \emph{negatives} made the student over-predict \emph{matches} more.
...
The zero pairwise overlap between the three added slices (Section~\ref{sec:results_layer2}) raises a natural question: if each strategy contributes different pairs, does their \emph{union} help where the parts did not? We built union training files on top of the shared base, namely the full union (base + string + LLM + web additions, deduplicated) and all pairwise combinations, and trained the full union under the standard three-seed protocol. The web component is the original slice of Section~\ref{sec:method_web}, not the v2 variants of Section~\ref{sec:iter_webv2}, which are significantly harmful on their own and would predictably drag the union down. The combined files are the largest training sets in the thesis: 7{,}900 pairs (23.6\% positive) on WDC Products and 37{,}187 pairs (18.9\% positive) on DBLP-Scholar, i.e.\ 3.2$\times$ and 2.2$\times$ the respective base sets.
...
The union experiment sharpens the thesis's central point. Even 5{,}400 additional WDC Products pairs from three sources with zero mutual overlap (perturbed copies, teacher-labeled hard negatives, and truly new web records) cannot outperform the cheapest of the three on its own. More pairs of the kinds that honest expansion can produce is not the binding constraint; the missing ingredient remains new in-distribution positives, which none of the three sources supplies (Section~\ref{sec:disc_mechanisms}).

\section{Low-Resource Ablation}
\label{sec:iter_lowresource}

The comparison with DistillER suggests a specific hypothesis: LLM-labeled data helps most when the base training set is too small to cover the common patterns: DistillER operates at 10\% of entities, and its largest gain from teacher labels occurs on its sparsest product dataset. Our WDC Products base set of 2{,}500 pairs may simply already be past that point. The ablation shrinks the base training set to 50\% and 25\% (random subsets, preserving the 20\% positive rate), combines each reduced base with the unchanged LLM-labeled slice from Section~\ref{sec:method_llm}, and measures whether the augmentation benefit grows as the base shrinks, the regime of Figure~4 of \citet{steiner2026labeling}. It does, and this is the one clear positive result of the thesis.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llccrl}
    \toprule
    \textbf{Dataset} & \textbf{Budget} & \textbf{Base only} & \textbf{+ LLM slice} & \textbf{$\Delta$F1} & \textbf{McNemar $p$} \\
    \midrule
    WDC & 100\% (2{,}500) & 0.659 $\pm$ 0.015 & 0.661 $\pm$ 0.020 & +0.002 & 0.356 (n.s.) \\
        & 50\% (1{,}250) & 0.606 $\pm$ 0.024 & \textbf{0.626 $\pm$ 0.013} & \textbf{+0.019} & \textbf{0.0002}$^{*}$ \\
        & 50\% + string control & 0.606 $\pm$ 0.024 & 0.607 $\pm$ 0.023 & +0.000 & 0.128 (n.s.) \\
        & 25\% (625) & 0.000 $\pm$ 0.000 & \textbf{0.580 $\pm$ 0.046} & +0.580 & 0.0048 (nominal) \\
    \midrule
    DBLP & 100\% (17{,}223) & 0.9559 $\pm$ 0.0019 & 0.9566 $\pm$ 0.0006 & +0.001 & 0.728 (n.s.) \\
         & 50\% (8{,}612) & 0.9502 $\pm$ 0.0032 & 0.9511 $\pm$ 0.0011 & +0.001 & 0.875 (n.s.) \\
         & 25\% (4{,}306) & 0.9448 $\pm$ 0.0031 & 0.9427 $\pm$ 0.0054 & $-$0.002 & 0.766 (n.s.) \\
    \bottomrule
  \end{tabular}
  \caption{Low-resource ablation: F1 of the student trained on a reduced base set, with and without the LLM-labeled slice (3 seeds; WDC Products runs class-weighted). The ``string control'' row adds a string-augmented slice matched to the LLM slice in size and class balance instead (Section~\ref{sec:iter_lowresource_control}). $^{*}$Survives Bonferroni correction. The 100\% rows repeat the main results for reference.}
  \label{tab:lowresource}
\end{table}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/low_resource_ablation.pdf}
  \caption{WDC Products: effect of the LLM-labeled additions as a function of the base training budget (3-seed mean $\pm$ std; annotations give the mean difference vs.\ the base). The same 2{,}100-pair slice that ties the baseline at the full budget helps significantly at half the budget and rescues an otherwise untrainable set at a quarter. The orange bar at the 50\% budget is the volume control: a string-augmented slice of matched size and class balance, which gains nothing over the base.}
  \label{fig:low_resource}
\end{figure}

Table~\ref{tab:lowresource} and Figure~\ref{fig:low_resource} show the outcome. On \textbf{WDC Products at 50\% budget}, the LLM slice turns 0.606 $\pm$ 0.024 into 0.626 $\pm$ 0.013: better on all three seeds, with McNemar $p = 0.0002$ on the seed-averaged predictions, comfortably below the Bonferroni threshold. This is the strongest positive effect in the thesis, and it comes from \emph{exactly the same} 2{,}100 teacher-labeled pairs that produce a statistical tie at the full budget (+0.002, $p = 0.356$). Nothing about the data changed; only the base against which it is added did.

\subsubsection*{Signal or Volume? A Matched Control}
\label{sec:iter_lowresource_control}

An obvious objection to the 50\% result is that it might reflect data \emph{volume} rather than label \emph{signal}: perhaps any 2{,}100 additional pairs would have helped a starved base. We tested this with a control arm designed to be as exactly comparable as possible: the halved base is expanded with 2{,}100 \emph{string-augmented} copies of its own pairs (label-preserving perturbations, Section~\ref{sec:method_string}), matched to the LLM slice not only in size but also in class balance. Both control and LLM files contain exactly 3{,}350 pairs with 467 positives (13.9\%), so even the class weights of the loss are identical. The only difference between the two arms is where the added pairs' content and labels come from.

The control changes nothing: 0.607 $\pm$ 0.023 versus the base's 0.606 $\pm$ 0.024 (+0.000, $p = 0.128$). Head-to-head, the LLM arm beats the volume control directly (McNemar $p = 0.0216$ on the seed-averaged predictions; ensemble F1 0.627 versus 0.644), though this secondary comparison is nominal under Bonferroni correction; the primary, correction-surviving evidence remains the LLM arm's $p = 0.0002$ against the base. One nuance is worth recording for completeness: the control's seed-averaged \emph{ensemble} F1 (0.627) sits slightly above the base's (0.611) even though its per-seed mean does not move, which is the variance-reduction effect of averaging three models rather than a property of the training data.

Taken together, the 50\% row now supports a clean attribution: \textbf{the low-resource gain is genuine teacher-label signal, not data volume.} A same-sized, identically balanced addition without new information does nothing; the teacher-labeled addition (new pairings, new hard negatives, and a few new positives) moves the student. This also sharpens the 25\% row retroactively: there the base collapses and any volume plausibly helps, which is why we report that row as trainability restoration; at 50\%, where the base trains fine, volume alone is demonstrably not the mechanism.

At \textbf{25\% budget} the effect changes character. The 625-pair base is too small for stable class-weighted fine-tuning: all three seeds collapse to the all-negative predictor (F1 = 0). Adding the LLM slice restores a working matcher at 0.580 $\pm$ 0.046. We report this as a \emph{data-volume} effect rather than evidence about label quality (almost any 2{,}100 additional pairs would presumably prevent the collapse), and note that its McNemar $p = 0.0048$ is only nominal under correction. The honest reading of the 25\% row is ``LLM expansion restores trainability'', not ``LLM expansion adds 0.58 F1 of signal''.

On \textbf{DBLP-Scholar}, the ablation is a flat null at every budget ($p = 0.77$--$0.88$). Even at a quarter of the data the base student still reaches 0.945 (the dataset is so far inside the ceiling regime that scarcity never bites within the tested range), and the LLM slice, being 0\% positive there, has no signal to contribute regardless.

The ablation resolves the thesis's central tension. The full-budget conclusion (Chapter~\ref{ch:results}) and DistillER's optimistic low-resource findings are not in conflict; they sit on two sides of a saturation boundary, and WDC Products crosses that boundary between 1{,}250 and 2{,}500 human-labeled pairs. Below it, a positive-starved, hard-negative-heavy teacher-labeled slice carries real marginal signal: the base set undersupplies decisive contrasts, and the additions fill actual gaps. Above it, the same slice is redundant with what the human pairs already teach. This directly confirms the saturation reading of DistillER's Walmart-Amazon result (Section~\ref{sec:comp_distiller_results}) and mirrors the shape of the learning curve of \citet{steiner2026labeling}: the value of machine-labeled data is a function of where you stand on the budget axis, not a fixed property of the data.
...
Two recent works study the same underlying question as this thesis (can an LLM teacher create training data for a small entity matching student?) and reach considerably more optimistic conclusions. This chapter takes both apart in detail and reconciles the difference. It also closes the loop opened in Section~\ref{sec:rw_positioning}: the F1 numbers collected in Table~\ref{tab:rw_overview} can only be read side by side once budgets, benchmark versions, and task framings are fixed, and fixing them is what this chapter does. Section~\ref{sec:comp_framework} fixes what can and cannot be compared. Sections~\ref{sec:comp_distiller} and~\ref{sec:comp_aaron} treat DistillER \cite{zeakis2026distiller} and \citet{steiner2026labeling} in turn, and Section~\ref{sec:comp_synthesis} draws the joint picture: the three studies do not contradict each other; they measure different regimes of the same underlying trade-off, and placing them side by side reveals a boundary that none of them shows alone.
...
\begin{figure}[tb]
  ...
  \caption{WDC Products F1 as a function of the number of training pairs (log scale). Purple: the machine-label learning curve of \citet{steiner2026labeling}, Active learning (ML) variant, digitized from their Figure~4 with the endpoint pinned to their Table~1 value (70.65) and the benchmark line at 71.94; read-off precision is about $\pm$0.5--1 F1 per point. The star marks their large-split human-label benchmark. Gray/green with error bars: this thesis's configurations at their actual training sizes, including the reduced-budget points of Section~\ref{sec:iter_lowresource}. Our human-label baseline at 2{,}500 pairs lies on their curve; at full budget none of our expansions moves meaningfully up it, while in the reduced-budget region the LLM additions produce visible gains. Growing an adequate training set with expandable pairs is not equivalent to growing it with benchmark-quality labeled pairs, but growing an inadequate one helps.}
  \label{fig:budget_curve}
\end{figure}

First, the 0.06 baseline gap is fully explained by budget: 0.659 at 2{,}500 pairs is \emph{on} their curve, not below it. Second, at a \emph{matched} budget of $\sim$2{,}500 labels, their machine-labeled student sits at roughly 0.55--0.62 on that same curve, i.e.\ \emph{below} our human-label baseline of 0.659. In other words: at our budget, 2{,}500 human labels beat 2{,}500 machine labels; machine labeling wins on cost only when it buys volume that humans cannot afford. That is their study, stated budget-aware (machine labels are cheap enough to climb the curve), and it coexists without contradiction with ours: at a fixed, already-labeled base, machine-labeled \emph{expansion} of a closed benchmark adds little, because the addable pairs are the wrong ones. The two studies describe the two ends of the same trade: \emph{scale} (their axis) versus \emph{marginal content} (ours).
...
\item \textbf{Base-set saturation gates the benefit, and we located the gate.} The one large machine-label win across all three studies' full-budget settings (DistillER's Walmart-Amazon, +0.13) occurs at extreme data starvation; at our 2{,}500-pair base the effect is absent, and at full DBLP-Scholar scale everyone's effects vanish. Our low-resource ablation (Section~\ref{sec:iter_lowresource}) confirms the gate directly on WDC Products: the same LLM additions that tie the baseline at 2{,}500 pairs help significantly at 1{,}250 and restore trainability at 625, and a volume-matched control shows the 1{,}250-pair gain is carried by the teacher's labels, not by extra data. LLM-teacher data behaves like a remedy for scarcity, not an amplifier of adequacy.
...
For a practitioner, the map reads: if you have almost no labels, let an LLM teacher label a similarity-ranked pool for you (DistillER's regime; strong evidence it works). If you can label the benchmark-scale pool, machine labels trade a point or two of F1 for a large cost saving (Steiner \& Bizer's regime). But if you already have a curated training set and hope an LLM will \emph{extend} it over the same entity universe (this thesis's regime), expect little, \emph{unless that training set is small}: below roughly half our WDC Products base budget, the same additions carry real marginal signal again (Section~\ref{sec:iter_lowresource}). The extendable positives mostly do not exist, and importing them from outside (the web) runs into a text-distribution wall that naive retrieval does not cross; what the additions can supply, hard negatives and variation, only has value where the human pairs do not already supply it.
...
The headline of this thesis, that automatic expansion did not improve the student at the full budget, would be uninteresting if it had a single cause or no cause. It has three, they are separable, and each was established by a different part of the evidence.

\textbf{Mechanism 1: positive scarcity in entity-disjoint pools.} A benchmark is, by construction, an attempt to enumerate the matches among its entities. Whatever match-finding process built the benchmark already harvested the discoverable positives into the labeled splits. An expansion pipeline that (correctly) excludes known pairs and (correctly) restricts itself to training entities therefore fishes in the leftovers: pairs that passed blocking similarity but were never labeled as matches, which are overwhelmingly true negatives. We measured this directly: 5.6\% positives in the WDC Products pool, zero discoverable positives on DBLP-Scholar, and consequently a 10.3\%- and 0\%-positive added slice. No selection algorithm and no teacher can label positives that are not there. The practical formulation: \emph{on a closed benchmark, honest expansion is structurally positive-starved; the accessible new information is almost entirely negative}. Note the qualifier ``closed'': the mechanism is about benchmarks whose entity universe is fixed and pre-mined, which is exactly the situation of academic evaluation, and \emph{not} necessarily of production settings where new entities stream in continuously.

\textbf{Mechanism 2: text-distribution mismatch in web-imported data.} Web retrieval gets around Mechanism 1 (it demonstrably injected new positive records, 81\% of its slice) and still failed, in the follow-up even significantly ($-$0.029 F1). The evidence localizes the failure in the text, not the labels: the imported records are web-styled (snippet truncation, marketing language, sparse attributes) while every test pair is benchmark-styled, and training on (clean, web) pairs pushed the student's boundary in a direction that hurt on (clean, clean) pairs: false positives rose steadily with the amount of web-styled data. Retrieval solves the \emph{content} problem and creates a \emph{style} problem. Any future web-augmentation design must translate retrieved records into the target distribution (or make the student style-invariant) before the added content can pay off.

\textbf{Mechanism 3: ceiling effects.} On DBLP-Scholar every intervention, including a perfectly targeted one (Section~\ref{sec:iter_hardneg}), returned null or a tiny regression. The dataset offers no room: the baseline sits at 0.956 with errors concentrated in a few dozen truly ambiguous pairs, the split is not entity-disjoint (93\% of test entities seen in training) and contains 186 text-identical cross-split pairs, and its structure supplies at most 92 examples of its own hardest error type. A ceiling dataset cannot judge between augmentation methods; it can only verify they do no harm. In retrospect, a mid-difficulty benchmark (e.g.\ Walmart-Amazon or Abt-Buy) would have been a more informative second dataset, a choice future work can correct cheaply.

The three mechanisms compose into the overall picture: string augmentation adds variation but no information (bounded by design), LLM expansion adds accurate labels over an information-poor pool (bounded by Mechanism 1), and web expansion adds information in the wrong style (bounded by Mechanism 2), while DBLP-Scholar caps everything from above (Mechanism 3). The low-resource ablation adds the boundary condition to all three: these ceilings bind only when the base training set already covers the common patterns. Shrink the base below that coverage point, and the same hard-negative-heavy LLM additions become significantly valuable (Section~\ref{sec:iter_lowresource}): Mechanism 1 limits what expansion can add, but whether that addition \emph{matters} is decided by what the base set lacks.
...
\textbf{Statistical resolution.} Three seeds bound our resolution at roughly $\pm$0.02 F1 on WDC Products; effects smaller than that are undetectable here. Claims are phrased as ``no detectable effect at $n{=}3$'', not ``no effect''. Relatedly, McNemar on seed-averaged predictions compares score-ensembles, not per-seed distributions (Section~\ref{sec:setup_significance}), and roughly seventeen comparisons were run; only the web-v2 hard-negative degradation and the 50\%-budget LLM gain survive Bonferroni; the DBLP string-augmentation regression, the WDC union result, the 25\%-budget comparison, and the LLM-versus-control comparison ($p = 0.0216$) are nominal.
...
This thesis asked whether an existing entity matching training set can be expanded automatically by string augmentation, by LLM-teacher labeling of actively selected new pairs, or by LLM-labeled web retrieval. The answer turned out to depend on where you stand: a careful no at the full labeling budget, and a clear yes once labels are scarce, together with an account of the mechanisms behind both halves.

Across two benchmarks and a three-seed protocol with paired significance tests, no strategy improved a RoBERTa student over its human-labeled baseline (RQ1--RQ3): string augmentation came closest on WDC Products (+0.016 mean F1, not significant) while slightly but consistently harming DBLP-Scholar; LLM-based and web-based expansion tied their baselines everywhere; and even the union of all three mutually non-overlapping additions only reproduced string augmentation's result (+0.017, statistically tied with string alone). The expanded training sets missed their design targets for positive ratio and corner-case share (RQ4), and the miss is structural rather than accidental: once expansion is honestly entity-disjoint, the candidate pool of a closed benchmark contains almost no undiscovered positives (5.6\% on WDC Products, none on DBLP-Scholar). The LLM teacher itself performed excellently (97.1\% label accuracy against cluster ground truth, with errors concentrated exactly where uncertainty sampling sent it), so the bottleneck is what there is to label, not how well it is labeled. Web retrieval, the one strategy that truly imports new positives, stumbled over a second wall: retrieved records are written in web style, benchmark test pairs in benchmark style, and a targeted second-iteration experiment showed that deepening the web-styled data (even with verified, high-quality generated hard negatives) makes the student significantly worse, pinning the failure on text distribution rather than label balance.

The low-resource ablation completes the picture and bounds the negative result. Cutting the WDC Products base set to half turns the LLM-labeled additions from a statistical tie into a significant gain (0.606 to 0.626 F1, $p = 0.0002$, one of only two Bonferroni-surviving effects in the thesis), and a control that instead adds a string-augmented slice of identical size and class balance gains nothing (+0.000): the gain is teacher-label signal, not data volume. Cutting the base to a quarter makes the additions the difference between a collapsed model and a working one. The same 2{,}100 teacher-labeled pairs are worthless at 2{,}500 human pairs and valuable at 1{,}250, so the operative question for automatic expansion is not ``does it work?'' but ``is the existing training set already past its coverage point?''. On our benchmarks that point lies between 1{,}250 and 2{,}500 pairs for WDC Products, while DBLP-Scholar never leaves the saturated regime within the tested range.

Set against DistillER and \citet{steiner2026labeling} (RQ5), these results are not a contradiction but a boundary, one our ablation locates empirically. Their replacement setting (the teacher relabels the benchmark's own pairs, positives included by construction) works, and works for the reason our audit confirms: modern LLM teachers are accurate annotators. Our expansion setting starves the same machinery of positives at full budget, yet recovers their optimistic conclusion exactly where their settings live: in the scarce-label regime. Together the three studies say: LLM teachers remedy label \emph{scarcity}; they do not conjure pair \emph{content} that a closed benchmark does not contain, and whether that limitation bites depends entirely on how much labeled data you already have.

\section{Future Work}
\label{sec:future}

The experimental program of the thesis is complete; the natural next step within it is to resolve the coverage point more finely. The ablation brackets WDC Products' saturation boundary between 1{,}250 and 2{,}500 human-labeled pairs with two budget levels; a denser budget grid (and a second mid-difficulty product benchmark) would turn the bracket into a curve and show whether the crossover behaves like the learning-curve knee of \citet{steiner2026labeling}.
...
{\sloppy This appendix reproduces the prompts used with the teacher model (\texttt{claude-sonnet-4-6}) in the LLM-based and web-retrieval pipelines. All prompts require a strict JSON answer; malformed responses were retried up to three times. \texttt{\{left\}} and \texttt{\{right\}} denote the serialized records of a pair.\par}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    llm\_aug & 36.9 & 35.2 & 35.7 & 13.0 & 20.7 & 31.8 \\
    web\_aug & 35.0 & 37.9 & 34.9 & 13.0 & 19.4 & 33.2 \\
    \bottomrule
  \end{tabular}
...
\noindent Mannheim, den 31.~Juli 2026 \hfill
