...
Paired significance testing for model-vs-model comparisons on a shared test set.
  ...
  - Cohen's h effect size on accuracy, so statistical significance is always 
    reported next to practical significance.
...
Usage:
    ...
    # reproduce the full comparison table
...
def load_preds(
    run: str, dataset: str, split: str = "test"
...
def mcnemar(true: np.ndarray, pred_a: np.ndarray, pred_b: np.ndarray) -> dict:
    ...
    p_exact = float(
        stats.binomtest(min(n10, n01), n=disc, p=0.5, alternative="two-sided").pvalue
    )
    return {
        "n10": n10,
        "n01": n01,
...
def compare(
    ...
    if not quiet:
        ...
        print(
            f"  F1 baseline   : {f1_base['f1']:.4f}  [{f1_base['ci_lo']:.4f}, {f1_base['ci_hi']:.4f}]"
        )
        print(
            f"  F1 treatment  : {f1_treat['f1']:.4f}  [{f1_treat['ci_lo']:.4f}, {f1_treat['ci_hi']:.4f}]"
        )
        print(
            f"  F1 difference : {diff['f1_diff']:+.4f}  [{diff['ci_lo']:+.4f}, {diff['ci_hi']:+.4f}]  (bootstrap p={diff['p_boot']:.4f})"
        )
        print(
            f"  McNemar       : n10={mc['n10']} n01={mc['n01']}  χ²(cc)={mc['chi2']}  p_exact={mc['p_exact']:.4f}  p_χ²={mc['p_chi2']:.4f}"
        )
...
def run_all(n_boot: int = 10000, seed: int = 0, seedmean: bool = True) -> list[dict]:
    ...
    for r in rows:
        ...
        print(
            f"{r['dataset']:<13} {comp:<28} {r['f1_diff']['f1_diff']:>+8.4f} {r['mcnemar']['p_exact']:>11.4f}  {r['verdict']}"
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Paired significance tests (McNemar + bootstrap F1)"
    )
    ...
    ap.add_argument(
        "--all",
        action="store_true",
        help="Run the standard Task 6 comparison table (3-seed seed-ensemble by default)",
    )
    ap.add_argument(
        "--single-seed",
        action="store_true",
        help="With --all: use single-seed run names instead of _seedmean (reproduces the superseded table)",
    )
    args = ap.parse_args()
